...
export default function GalleryItem(props: any) {
  ...
  useEffect(() => {
    if (params.id) {
      fetchMapArray(params.id).then((res) => {
        if (res?.frontArray != null) {
  ...
  async function fetchMapArray(tokenId: any) {
    ...
    if (worlds != null && tokenId) {
      try {
        ...
        const frontArray = frontBlockArray(_mapArray);
