...
> = (props: React.PropsWithChildren<{ children: any }>): React.ReactElement => {
  ...
  const revComposeD = (compMap : string) => {
    ...
    while (i < compMapSplit.length)
    {
      ...
      if (compMapSplit[i] == "0") {
        ...
        tempArray[y][x].id = parseInt(compMapSplit[i][3] + compMapSplit[i][4] + compMapSplit[i][5] + compMapSplit[i][6]);
        tempArray[y][x].state = parseInt(compMapSplit[i][7]);
        tempArray[y][x].blockType = parseInt(compMapSplit[i][8]);
        tempArray[y][x].blockFertility = parseInt(compMapSplit[i][9] + compMapSplit[i][10]);
      }
    ...
    const   generateFullMap = () => {
      ...
      let tree : any[] = [48, 64, 84, 87, 91, 95, 99, 103, 106, 113, 126, 128, 129, 130, 133, 134, 143, 144, 147, 148, 149, 150, 152, 154, 163, 164, 165, 168, 171, 184, 185, 189, 190, 191, 196, 204, 205, 207, 210, 211, 216, 218, 223, 225, 226, 227, 228, 230, 231, 232, 236, 243, 244, 249, 250, 253, 256, 257, 258, 265, 266, 267, 268, 271, 277, 284, 287, 294, 301, 308, 309, 311, 313, 316, 323, 325, 327, 329, 330, 331, 348, 350, 352, 353, 354, 357, 366, 371, 373, 376, 382, 391, 392, 394, 395, 403, 404, 407, 408, 409, 410, 412, 414, 421, 424, 426, 427, 432, 434, 444, 446, 448, 449, 450, 451, 452, 456, 459, 469, 471, 473, 475, 485, 488, 489, 502, 507, 510, 525, 528, 533, 536, 540, 545, 557, 562, 566, 571, 577, 584, 591, 595]
      let rock : any[] = [53, 66, 68, 85, 89, 122, 162, 166, 173, 181, 234, 312, 315, 328, 339, 345, 359, 367, 368, 369, 436, 439, 514, 526, 537, 538, 550, 589];
      let bush : any[] = [92,111,145,151,169,187,201,229,246, 248, 272, 274, 288, 291, 293, 305, 364, 411, 429, 477, 482, 492, 494, 495, 552];
      ...
      while (i < 640)
      {
        ...
        fullMap = fullMap + parseInt(infraType.toString() + resType.toString() + type.toString() + tempUID.toString() + state.toString() + fertility.toString()).toString() + "|";
  ...
  const checkResBuild = (id : number, inventory : any, fixBuildVal : any) => {
    ...
    {
      if (inventory[i] < fixBuildVal[id].createCost[i])
      {
  ...
  const initGameSession = React.useCallback(
    (inventory: Iinventory, land: ILand, playerActions: [], playerBuildings: []) => {
      ...
      let staticBuildings : any[] = [];
      getStaticBuildings().then((res) => {
        staticBuildings = res;
      });
      //console.log("staticBuildings = ", staticBuildings);

      // Fetch static resources spawned
      let staticResources : any[] = [];
      getStaticResources().then((res) => {
        staticResources = res;
      });
      ...
      while (i < staticBuildings.length)
      {
        ...
        fixBuildVal[i].createCost = parseResToArray(staticBuildings[i].createCost);
        fixBuildVal[i].maintainCost = parseResToArray(staticBuildings[i].maintainCost);
        fixBuildVal[i].production = parseResToArray(staticBuildings[i].production);
        i++;
      }
      ...
      while (i < staticResources.length)
      {
        ...
        fixResVal[i] = [];
        fixBuildVal[i].id = staticResources[i].id;
        fixBuildVal[i].biome = staticResources[i].biomeId;
        fixBuildVal[i].type = staticResources[i].type;
        fixBuildVal[i].nbHarvest = staticResources[i].nbHarvest;
        fixBuildVal[i].animated = staticResources[i].animated;
        fixBuildVal[i].locked = staticResources[i].locked;
        fixBuildVal[i].size = staticResources[i].size;
        fixBuildVal[i].level = staticResources[i].nbLevels;
        fixBuildVal[i].fertility = staticResources[i].fertilityNeed;
        fixResVal[i].sprites = parseResToArray(tempSpriteArray[0]);
        fixResVal[i].harvestSprites = parseResToArray(tempSpriteArray[1]);
        fixResVal[i].harvestCost = parseResToArray(staticResources[i].harvestCost);
        fixResVal[i].production = parseResToArray(staticResources[i].production);
