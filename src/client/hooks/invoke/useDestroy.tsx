...
export default function useDestroy() {
  ...
  return useCallback(async (tokenId : number, pos_start: number, building_type_id: number, posX: number, posY: number, uniqueId: number) => {
    if (!contract || !account) {
    ...
    return contract
      .invoke('destroy', [uint256.bnToUint256(tokenId as number), pos_start])
