...
export function FrameItem(props : any) {
  ...
  return (
    <div className="flex flex-row justify-center inline-block relative">
        ...
        <div className={`fontHPxl-sm ${props.content[1] >= resources[props.content[0]] && props.option == 1 ? `fontRed` : ``}`} style={{marginTop: '25px', marginLeft: '-20px'}}>{props.inputFuel ? props.content[1] * props.inputFuel : props.content[1]}</div>
    </div>
  )
}